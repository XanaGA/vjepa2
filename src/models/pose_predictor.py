#!/usr/bin/env python3
#
# Pose-conditioned Vision Transformer predictor.
#
...
class VisionTransformerPosePredictor(nn.Module):
    """Pose-conditioned Vision Transformer Predictor.

    This closely follows `VisionTransformerPredictorAC`, but is intended
    for conditioning on camera poses instead of robot actions/states.
    """
    ...
    def __init__(
        ...
        use_rope=True,
        action_embed_dim=16,  # default: flattened 4x4 pose
    ...
    def forward(self, x, actions, states):
        """
        :param x: context tokens (frames except last one)
        :param actions: pose at time t (typically the last frame pose),
                        broadcast or aligned to context length
        :param states: previous poses (all poses before time t)
        """
        ...
        # Ensure actions/states have matching temporal length T
        # actions: [B, T_a, C] or [B, 1, C] -> [B, T, C]
        if actions.size(1) == 1 and T > 1:
            actions = actions.expand(-1, T, -1)
        elif actions.size(1) != T:
            raise ValueError(f"actions length {actions.size(1)} does not match context T={T}")

        # states: [B, T_s, C] -> [B, T, C] (pad or crop as needed)
        if states.size(1) < T:
            # pad last state
            pad_states = states[:, -1:, :].expand(-1, T - states.size(1), -1)
            states = torch.cat([states, pad_states], dim=1)
        elif states.size(1) > T:
            states = states[:, :T, :]

        # Interleave pose tokens
        ...
        # Split out pose and frame tokens
...
def vit_pose_predictor(**kwargs):
    model = VisionTransformerPosePredictor(
        mlp_ratio=4,
        qkv_bias=True,
